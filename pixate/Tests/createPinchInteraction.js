...
Pixate.ApiTest.bundle({
	createPinchInteraction: [{
		...
		test: function(Assert) {
			...
			Assert.isNullOrUndefined(layer.interactions.Pinch, 'Expected that Pinch interaction wouldnt exist');
			Assert.areEqual(0, layer.animations.length, 'Expected that the layer would not contain any animations');
			
			var interaction = Pixate.createPinchInteraction(layer);

			if (!Assert.isNotNullOrUndefined(interaction, 'Expected interaction to exist')) {
				return;
			}

			Assert.areEqual(Pixate.Api.Types.Interaction.Pinch.type, interaction.type, 'Expected interaction type to be Pinch');
			...
			if (Assert.areEqual(1, layer.animations.length, 'Expected that a default animation would have been created')) {
				...
				var defaultAnimation = Pixate.Api.Types.Interaction.Pinch.events.pinch.defaultAnimation;
		...
		test: function(Assert) {
			var interaction = Pixate.createPinchInteraction();
		...
		test: function(Assert) {
			var layer = Pixate.createLayer('layer');
			var interactionOne = Pixate.createPinchInteraction(layer);
			var interactionTwo = Pixate.createPinchInteraction(layer);
