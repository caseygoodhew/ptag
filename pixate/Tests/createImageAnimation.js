...
Pixate.ApiTest.bundle({
	createImageAnimation: [{
		...
		test: function(Assert) {
			...
			var animation = Pixate.createImageAnimation(layer, Pixate.basedOn(layer, 'tap'));
			...
			if (Assert.isNotNullOrUndefined(animation, 'Expected animation to exist')) {
				...
				Assert.areEqual(Pixate.Api.Types.Animation.Image.type, animation.type, 'Unexpected animation type "'+animation.type+'"');
				Assert.areEqual(Pixate.Api.Types.Animation.Image.defaultName, animation.name, 'Unexpected animation name "'+animation.name+'"');
		...
		test: function(Assert) {
			var animation = Pixate.createImageAnimation(null, { });
		...
		test: function(Assert) {
			...
			var animation = Pixate.createImageAnimation(layer);
