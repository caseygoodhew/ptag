...
Pixate.ApiTest.bundle({
	createColorAnimation: [{
		...
		test: function(Assert) {
			...
			var animation = Pixate.createColorAnimation(layer, Pixate.basedOn(layer, 'tap'));
			...
			if (Assert.isNotNullOrUndefined(animation, 'Expected animation to exist')) {
				...
				Assert.areEqual(Pixate.Api.Types.Animation.Color.type, animation.type, 'Unexpected animation type "'+animation.type+'"');
				Assert.areEqual(Pixate.Api.Types.Animation.Color.defaultName, animation.name, 'Unexpected animation name "'+animation.name+'"');
		...
		test: function(Assert) {
			var animation = Pixate.createColorAnimation(null, { });
		...
		test: function(Assert) {
			...
			var animation = Pixate.createColorAnimation(layer);
