...
Pixate.ApiTest.bundle({
	createScaleAnimation: [{
		...
		test: function(Assert) {
			...
			var animation = Pixate.createScaleAnimation(layer, Pixate.basedOn(layer, 'tap'));
			...
			if (Assert.isNotNullOrUndefined(animation, 'Expected animation to exist')) {
				...
				Assert.areEqual(Pixate.Api.Types.Animation.Scale.type, animation.type, 'Unexpected animation type "'+animation.type+'"');
				Assert.areEqual(Pixate.Api.Types.Animation.Scale.defaultName, animation.name, 'Unexpected animation name "'+animation.name+'"');
		...
		test: function(Assert) {
			var animation = Pixate.createScaleAnimation(null, { });
		...
		test: function(Assert) {
			...
			var animation = Pixate.createScaleAnimation(layer);
